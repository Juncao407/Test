def preprocess_dataframe(df, first_cols, split):
    ...
    for col in split:
        ...
        temp = pd.get_dummies(temp.apply(pd.Series), prefix='', prefix_sep='')
        # 使用更现代的方法替代已弃用的 groupby(axis=1)
    ...
    # 确保first_cols中的列确实存在于DataFrame中
    ...
    cols_na = df.select_dtypes(include=[np.number]).columns.difference(existing_first_cols)
    df[cols_na] = df[cols_na].fillna(0)

    # Find categorical columns to perform one-hot encoding, excluding 'first_cols'
    categorical_cols = df.select_dtypes(include=[object]).columns.difference(existing_first_cols).tolist()
